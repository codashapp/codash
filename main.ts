  ...
  fs.readFile(__dirname + `/data/${lang}.jsonc`, (err, data) => {
    if (err)
      return res
        .status(500)
        .send(
          "Internal server error. Pak is either not available or could not be found."
        );
    else {
...
app.get("/api/internal/overview", async (req, res) => {
  ...
  !country_data ??
    res
      .status(400)
      .send(
        `Invalid country code ${country_data}. Please make sure that this country is supported by CoDash, and that the code is valid.`
      );
  ...
  let yesterday_filter = country_data.data.filter((obj: { date: string }) => {
    ...
    return today_filter.date == today
      ? obj.date == yesterday
      : obj.date == twodays;
  });
